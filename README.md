...
- 项目结构从单文件 HTML 优化为 `index.html`、`style.css`、`script.js` 三件套，方便维护、部署和平台上传。
- 新增 `images/logo.png` 与 `images/banner.jpg`，用于平台封面和横幅展示。
- 优化多设备适配：支持 PC、平板和手机横屏体验，手机竖屏时会提示旋转到横屏。
- 优化移动端性能：手机端会降低粒子和爆炸碎屑数量，减少低端设备卡顿。
- 调整电脑端操作：鼠标左键攻击、右键移动，同时保留 WASD / 方向键移动。
- 无双大招改为每局仅可使用一次，释放后进入 15 秒双刀乱舞与吸血状态。
- 增强积分规则：按击杀、怪物类型、当前波数和连续击杀倍率统计分数。
- 保留新版主菜单、五张地图选择页、9 种武器、5 级 BOSS、实时音效和背景音乐。
